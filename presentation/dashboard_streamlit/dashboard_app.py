...
DATA_ROOT = PROJECT_ROOT / 'sorties_intermediaires'
STATIC_DASHBOARD_DIR = PROJECT_ROOT / 'presentation' / 'dashboards_statiques'
DATA_FILES = {
    'data_national.csv': DATA_ROOT / 'national' / 'data_national.csv',
    'national_forecast.csv': DATA_ROOT / 'previsions' / 'national_forecast.csv',
    'data_local.csv': DATA_ROOT / 'local' / 'data_local.csv',
    'data_international.csv': DATA_ROOT / 'international' / 'data_international.csv',
    'regional_positivity_monthly.csv': DATA_ROOT / 'regional' / 'regional_positivity_monthly.csv',
}
DATA_DIR = DATA_ROOT

def data_file(filename):
    return DATA_FILES.get(filename, STATIC_DASHBOARD_DIR / filename)
...
def load_data_with_mtimes(mtimes):
    ...
    nat = pd.read_csv(data_file('data_national.csv')) if data_file('data_national.csv').exists() else pd.DataFrame()
    fc = pd.read_csv(data_file('national_forecast.csv')) if data_file('national_forecast.csv').exists() else pd.DataFrame()
    local_df = pd.read_csv(data_file('data_local.csv')) if data_file('data_local.csv').exists() else pd.DataFrame()
    intl_df = pd.read_csv(data_file('data_international.csv')) if data_file('data_international.csv').exists() else pd.DataFrame()
...
files_check = {
    'data_national.csv': data_file('data_national.csv').exists(),
    'national_forecast.csv': data_file('national_forecast.csv').exists(),
    'data_local.csv': data_file('data_local.csv').exists(),
    'data_international.csv': data_file('data_international.csv').exists(),
}
...
nat_file = data_file('data_national.csv')
try:
    if nat_file.exists():
...
with tabs[0]:
    ...
    if not nat.empty:
        ...
        heat_csv = data_file('regional_positivity_monthly.csv')
        if heat_csv.exists():
            try:
...
with tabs[1]:
    ...
    try:
        if local_df is None or local_df.empty:
            local_path = data_file('data_local.csv')
...
with tabs[2]:
    ...
    try:
        if intl_df is None or intl_df.empty:
            intl_path = data_file('data_international.csv')
